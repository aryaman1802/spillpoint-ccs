...
# 3. SIMPLIFIED REWARD: +1 for no leakage, 0 for leakage (episode ends immediately)
# 4. OPTIMIZED GRAVITY CACHING: Reduces computation overhead
# 5. FIXED SEED: Same geology for ALL episodes
# """
...
#     5. SIMPLIFIED REWARD: +1 for no leakage, 0 for leakage (episode ends immediately)
...
#         # 5. Check for leakage (terminates episode)
#         has_leaked = leaked_m3 > 1e-3
        
#         # 6. SIMPLIFIED REWARD: +1 for no leakage, 0 for leakage
#         if has_leaked:
#             step_reward = 0.0
#             terminated = True
#         else:
...
#         Layout (2 rows):
#             Top row:  cross-section with region shading + CO2 plume
#             Bottom row: region capacity bars | timelapse gravity | borehole gravity
...
#         gs = gridspec.GridSpec(2, 3, height_ratios=[2, 1], hspace=0.35, wspace=0.35)

#         # ---- Top row: cross-section spanning all columns ----
...
#         region_colors = ['#cce5ff', '#d4edda', '#fff3cd']
#         region_labels = ['Region 1', 'Region 2', 'Region 3']
#         for mask, col, lbl in zip(
#                 [sim.mask1, sim.mask2, sim.mask3],
#                 region_colors, region_labels):
...
#         # Caprock
#         ax_cs.plot(phys_x, phys_top, 'k', lw=2, label='Caprock')
...
#         # Injectors
#         inj_colors = ['#0d6efd', '#fd7e14', '#6f42c1']
#         for i, (region, loc_m) in enumerate(self.injector_locs_m.items()):
#             ax_cs.axvline(loc_m, color=inj_colors[i], linestyle='--',
#                          linewidth=1.8, label=f"Injector R{region}")
...
#         ax_cs.invert_yaxis()
#         ax_cs.legend(loc="lower left", fontsize='small', ncol=2)
#         ax_cs.grid(True, alpha=0.2)

#         # ---- Bottom-left: region capacity bar chart ----
#         ax_bar = fig.add_subplot(gs[1, 0])

#         r1_mm3 = scaler.to_phys_vol(sim.v_reg[1]) / 1e6
#         r2_mm3 = scaler.to_phys_vol(sim.v_reg[2]) / 1e6
#         r3_mm3 = scaler.to_phys_vol(sim.v_reg[3]) / 1e6
#         cap1_mm3 = scaler.to_phys_vol(sim.cap1) / 1e6
#         cap2_mm3 = scaler.to_phys_vol(sim.cap2) / 1e6
#         cap3_mm3 = scaler.to_phys_vol(sim.cap3) / 1e6

#         x_pos = np.arange(3)
#         caps = [cap1_mm3, cap2_mm3, cap3_mm3]
#         fills = [r1_mm3, r2_mm3, r3_mm3]
#         bar_colors = ['#0d6efd', '#198754', '#ffc107']

#         ax_bar.bar(x_pos, caps, color='lightgrey', edgecolor='grey',
#                    linewidth=0.8, label='Capacity', width=0.6)
#         ax_bar.bar(x_pos, fills, color=bar_colors, alpha=0.85,
#                    label='Filled', width=0.6)
#         for i in range(3):
#             pct = (fills[i] / caps[i] * 100) if caps[i] > 0 else 0
#             ax_bar.text(i, fills[i] + 0.3, f"{fills[i]:.1f}\n({pct:.0f}%)",
#                        ha='center', va='bottom', fontsize=8)
#         ax_bar.set_xticks(x_pos)
#         ax_bar.set_xticklabels(['R1', 'R2', 'R3'])
#         ax_bar.set_ylabel("Volume (Mm³)")
#         ax_bar.set_title("Region Fill Status")
#         ax_bar.set_ylim(0, max(caps) * 1.35)
#         ax_bar.grid(axis='y', alpha=0.3)
#         ax_bar.legend(fontsize='small')

#         # ---- Bottom-center: timelapse gravity ----
...
#         # ---- Bottom-right: borehole gravity ----
#         ax_bh = fig.add_subplot(gs[1, 2])
#         bh_z, bh_gx, bh_gz = sim.get_borehole_gravity()
#         ax_bh.plot(bh_gz, bh_z, 'g-o', markersize=3, linewidth=1.5,
#                    label=r'$g_z$ (Vert)')
#         ax_bh.plot(bh_gx, bh_z, 'orange', linestyle='--', marker='x',
#                    markersize=3, linewidth=1.5, label=r'$g_x$ (Horiz)')
#         ax_bh.set_title("Borehole Gravity")
#         ax_bh.set_xlabel(r"$\Delta g$ ($\mu$Gal)")
#         ax_bh.set_ylabel("Depth (m)")
#         ax_bh.invert_yaxis()
#         ax_bh.grid(True, alpha=0.3)
#         ax_bh.legend(fontsize='x-small')
...
#                                grav_ylim=grav_ylim, figsize=figsize)
#         plt.show()
